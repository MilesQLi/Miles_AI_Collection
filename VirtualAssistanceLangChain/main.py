...
# pip install langchain langchain_community langchain_openai faiss-cpu ollama duckduckgo-search python-dotenv pydantic==1.10.11 dateparser tk requests beautifulsoup4 PyYAML pymupdf
...
import os  # For FAISS path check and PDF tool
...
from langchain_core.prompts import ChatPromptTemplate, \
    MessagesPlaceholder  # ChatPromptTemplate might not be directly used by new agent
...
# LLM Configuration
LOCAL_API_BASE = "http://localhost:1234/v1"#"http://localhost:1234/v1"  # Standard Ollama API endpoint (e.g., LM Studio)
...
try:
    ...
    messagebox.showerror("LLM Error",
                         f"Could not connect to LLM at {LOCAL_API_BASE}. Please ensure Ollama is running and the model '{LOCAL_MODEL_NAME}' is available.\nError: {e}")
    exit()
...
try:
    ...
    messagebox.showerror("Embedding Error",
                         f"Could not initialize Ollama embeddings with model '{EMBEDDING_MODEL_NAME}' at {OLLAMA_BASE_URL}.\nError: {e}")
    exit()
...
def load_vector_store():
    ...
    if os.path.exists(FAISS_INDEX_PATH):
        try:
            ...
            vector_store = FAISS.from_texts(["Initial knowledge base entry: Assistant started."], embedding=embeddings,
...
class CalendarEventInput(BaseModel):
    ...
    event_datetime_str: str = Field(...,
                                    description="The date and time of the event (e.g., 'tomorrow at 3 pm', 'next Friday at 10am'). Should be a specific, parsable date and time.")
...
def add_calendar_event(event_content: str, event_datetime_str: str) -> str:
    ...
    try:
        event_dt = dateparser.parse(event_datetime_str,
...
class CalendarRetrieveInput(BaseModel):
    start_datetime_str: str = Field(...,
                                    description="The start date/time for retrieving events (e.g., 'today', 'next Monday').")
    end_datetime_str: str = Field(...,
                                  description="The end date/time for retrieving events (e.g., 'end of today', 'next Friday').")
...
def retrieve_calendar_events(start_datetime_str: str, end_datetime_str: str) -> str:
    ...
    try:
        start_dt = dateparser.parse(start_datetime_str,
                                    settings={'PREFER_DATES_FROM': 'current_period', 'RETURN_AS_TIMEZONE_AWARE': False})
        end_dt = dateparser.parse(end_datetime_str,
                                  settings={'PREFER_DATES_FROM': 'current_period', 'RETURN_AS_TIMEZONE_AWARE': False})
        ...
        if start_dt > end_dt:  # If range is inverted, assume user means from start_dt to end of that day
            ...
            logging.info(
                f"End date was before start date for calendar retrieval. Adjusted end date to end of start day: {end_dt.isoformat()}")
        ...
        logging.info(
            f"Retrieved {len(found_events)} calendar events from DB between {start_dt.isoformat()} and {end_dt.isoformat()}")
...
# NEW: Pydantic model for the PDF reading tool's input
class ReadPdfInput(BaseModel):
    pdf_path: str = Field(...,
                          description="The full local file path to the PDF file to be read. For example: 'D:/pdfs/a.pdf' or '/home/user/documents/report.pdf'.")


# NEW: Tool to read text from a PDF file
@tool("read_pdf_text", args_schema=ReadPdfInput)
def read_pdf_text(pdf_path: str) -> str:
    """
    Reads the text content from a local PDF file specified by its full path.
    Use this to answer questions based on the content of a PDF document.
    """
    if not os.path.exists(pdf_path):
        return f"Error: The file path '{pdf_path}' does not exist. Please provide a valid and full file path."
    if not pdf_path.lower().endswith('.pdf'):
        return f"Error: The file at '{pdf_path}' is not a PDF file."
    ...
    try:
        ...
        if not full_text.strip():
            return f"Successfully opened '{os.path.basename(pdf_path)}' but found no extractable text content inside."

        logging.info(f"Successfully extracted text from PDF: {pdf_path}")
        # The agent receives the full text and can then summarize or answer questions.
        # For huge PDFs, this could exceed context limits, but it's a direct way to handle the request.
        return f"Successfully extracted the following text from '{os.path.basename(pdf_path)}':\n\n{full_text}"
    except Exception as e:
        logging.error(f"Failed to read PDF file at {pdf_path}: {e}", exc_info=True)
        return f"Error: An unexpected error occurred while reading the PDF file. Details: {e}"
...
tools = [
    ...
    read_pdf_text,  # NEW: Add PDF tool to the agent's toolkit
]

# System prompt text for LangGraph ReAct Agent
# The original agent_prompt is not directly used in the same way.
SYSTEM_PROMPT_TEXT = (
    f"You are '{HOST_NAME}', a helpful personal assistant for '{USER_NAME}'. The current time is {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}. Use tools when necessary, including a tool to read text from local PDF files."
)
...
class VirtualAssistantApp:
    def __init__(self, root_tk):
        ...
        self.calendar_header = tk.Label(self.calendar_frame, text="Upcoming Events (Next 30 Days)",
                                        font=('Arial', 10, 'bold'))
        ...
        self.refresh_calendar_btn = tk.Button(self.calendar_frame, text="Refresh Calendar",
                                              command=self.manual_refresh_calendar)
        ...
        self.add_message("System",
                         f"Assistant initialized. Type 'clear' to reset chat. Current time: {datetime.now().strftime('%Y-%m-%d %H:%M')}\n",
                         tag='system')
    ...
    def _load_calendar_events_from_db(self):
        ...
        try:
            ...
            logging.info(
                f"Loaded {len(calendar_events)} events from DB into memory. Next event ID calculated as {next_event_id}.")
        except Exception as e:
    ...
    def process_user_input(self, user_input):
        ...
        try:
            ...
            if use_agent and langgraph_agent_app and checkpointer:  # Check for langgraph_agent_app and checkpointer
                ...
                current_message_count = len(
                    current_graph_state.get('channel_values', {}).get("messages", [])) if current_graph_state else 0
                ...
                for msg in final_messages[current_message_count:]:
                    ...
                    if isinstance(msg, AIMessage) and msg.tool_calls:
                        for tc in msg.tool_calls:
                            ...
                            if tc.get('name') in ["add_calendar_event", "delete_calendar_event",
                                                  "update_calendar_event"]:  # Add other relevant tool names
                ...
                logging.info(
                    f"Invoking LLM directly for input: {user_input} (Tools disabled: {not use_agent}, Agent not init: {not langgraph_agent_app})")
                current_history = memory.load_memory_variables({})['chat_history']
                # Use a simple system message for direct calls
                direct_system_msg = SystemMessage(
                    content=f"You are '{HOST_NAME}', a helpful assistant for '{USER_NAME}'. The current time is {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}. Please respond directly to the user.")
        ...
        self.root.after(0, self.display_response_and_refresh, response_text, error_occurred,
                        calendar_potentially_modified)
            ...
            if checkpointer and isinstance(checkpointer, MemorySaver):
                ...
                if LANGGRAPH_THREAD_ID in checkpointer.storage:
                    try:
                        ...
                        logging.info(
                            f"Cleared LangGraph agent state for thread '{LANGGRAPH_THREAD_ID}' from MemorySaver.")
                    except Exception as e:
                ...
                logging.warning(
                    "Checkpointer is not MemorySaver, manual clearing of thread state might be needed or not supported directly.")
    ...
    def background_calendar_checker(self):
        ...
        try:
            ...
            self.message_queue.put(
                {"sender": "System", "message": "Error: Calendar reminder LLM failed to init.", "tag": "error"})
            return
            ...
            try:
                ...
                for event in events_to_remind:
                    try:
                        ...
                        messages = [SystemMessage(content=f"You are {HOST_NAME}."),
                                    HumanMessage(content=reminder_prompt)]
    ...
    def check_message_queue(self):
        while not self.message_queue.empty():
            try:
                ...
                self.add_message(msg_data.get("sender", "System"), msg_data.get("message", ""),
                ...
                if msg_data.get("tag") == 'assistant' and msg_data.get("sender") == HOST_NAME:
                    memory.save_context({
                                            "input": f"[Reminder Sent for Event ID {msg_data.get('event_id', 'N/A')}: {msg_data.get('message')}]"},
                                        {"output": ""})  # Log reminder as user input, assistant output is implicit
            except Exception as e:
    ...
    def refresh_calendar_display(self):
        ...
        if not upcoming_display_events_ui:
            self.calendar_display.insert(tk.END, "No upcoming events in the next 30 days.")
        else:
